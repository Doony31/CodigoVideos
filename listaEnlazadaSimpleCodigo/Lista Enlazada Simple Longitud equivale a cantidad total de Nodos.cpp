// LONGITUD EQUIVALE A CANTIDAD DE NODOS
...
private:
	Nodo<T>* cabeza;
	// Longitud representa la CANTIDAD TOTAL de nodos (0, 1, 2, ...)
...
public:
	...
	// Simplemente retornamos la variable
	uint obtenerTamano() {
		return longitud;
	}
};
...
int main() {
	...
	cout << "Lista de flotantes (Tamano " << list->obtenerTamano() << "): ";
	list->recorrerLista();

	cout << "Lista de strings (Tamano " << list2->obtenerTamano() << "): ";
	list2->recorrerLista();

	cout << "\nAgregando al final y eliminando en el medio...\n";
	list->agregarFinal(1.0);
	list->EliminarNodoPos(1);
	cout << "Nueva lista flotantes (Tamano " << list->obtenerTamano() << "): ";
	list->recorrerLista();
